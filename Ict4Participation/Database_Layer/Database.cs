        ...
        */
                              ...
                              string description, bool urgent, string location, int maxHulpverlener, int status)
        {
            using (OracleConnection c = new OracleConnection(@connectionstring))
            {
                ...
                c.Open();
                OracleCommand cmd = new OracleCommand(
                 "UPDATE \"Question\" SET \"Title\" = :A, \"StartDate\" = TO_DATE(:B, 'DD-MON-YYYY HH24:MI:SS'), \"EndDate\" = TO_DATE(:C, 'DD-MON-YYYY HH24:MI:SS'), \"Description\" = :D, \"Urgent\" = :E , \"Location\" = :F, \"AmountACCs\" = :G, \"Status\" = :H WHERE \"ID\" = :I ");
                cmd.Parameters.Add(new OracleParameter("A", title));
            ...
            bool driverLicense, bool HasCar, string uitschrijvingsdatum, bool ov, string Bday, string Picture, string Sex, string VOG)
        {
            using (OracleConnection c = new OracleConnection(@connectionstring))
            {
                ...
                OracleCommand cmd = new OracleCommand("INSERT INTO \"Acc\" (\"Gebruikersnaam\" ,\"Wachtwoord\" ,\"Email\" ,\"Naam\" ,\"Adres\" ," +
                    ...
                    "\"Foto\" ,\"VOG\", \"Geslacht\") " +
                   "VALUES(:un, :ph, :em, :na, :loc, :vil, :phon, :dl, :car, TO_DATE(:ud, 'DD-MON-YYYY HH24:MI:SS'), :ov, TO_DATE(:bd, 'DD-MON-YYYY HH24:MI:SS'), :pic, :vog, :ges)");
                cmd.Parameters.Add(new OracleParameter("un", Username));
                cmd.Parameters.Add(new OracleParameter("ph", PassHash));
                cmd.Parameters.Add(new OracleParameter("em", Email));
                cmd.Parameters.Add(new OracleParameter("na", Name));
